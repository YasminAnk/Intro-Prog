...
void faz_horizontal(int ** comeco, int *tam ,int ** fim, int ** linha, int ** coluna, int i, int n, int m, char ** matriz, char ** palavras, int d){
    int ocupado = 0;
...
void faz_vertical(int ** comeco, int * tam ,int ** fim, int ** linha, int ** coluna, int i, int n, int m, char ** matriz, char ** palavras, int d){
   int ocupado = 0;
    //variavel que diz se o local sorteado esta livreint ocupado = 0;
    for(int j = 0; j <= i; j ++){
        ...
        if(fim[i][0] > n || fim[i][1] > m || fim[i][0] < 0 || fim[i][1] < 0){
            ocupado = 1;
        }
        ...
        if(coluna[j][0] == comeco[i][1]){
            ...
            if((n - linha[j][1]) < (tam[i] - 1) && linha[j][0] < (tam[i] - 1)) //se não tiver espaco depois da coluna ou antes
                ocupado = 1;
        }
            ...
            if(nivel==1){
                for (int i = 0; i < qtd; i++){ 
                    ...
                    if (pos[i] == 0){
                        ...
                        ,fim, linOcupada, colOcupada, i, n, m, matriz, palavras, d);
                    } else{
                        // palavra na vertical
